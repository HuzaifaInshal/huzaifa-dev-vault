...
export function Topbar({ pages }: TopbarProps) {
  ...
  const primaryLinks = [
    navTree.root
      ...
      : { label: 'Docs', path: '/' },
    ...navTree.nodes.slice(0, 5).map((node) => ({ label: node.label, path: node.path })),
  ]

  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-zinc-800/60 bg-zinc-950/90 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-[1680px] items-center justify-between gap-6 px-6 lg:px-8">
        ...
        <div className="hidden flex-shrink-0 items-center gap-3 lg:flex">
          <div className="flex h-9 w-64 items-center rounded-lg border border-zinc-800 bg-zinc-900 px-3 text-sm text-zinc-500">
