...
function formatRangeLabel(value?: DateTimeRangeValue, placeholder = 'Select Date & Time Range') {
  if (!value?.startDateTime && !value?.endDateTime) return placeholder
  if (value?.startDateTime && !value?.endDateTime) return `${formatDateTimeLabel(value.startDateTime)} - ...`
  if (!value?.startDateTime || !value?.endDateTime) return placeholder
  return `${formatDateTimeLabel(value.startDateTime)} - ${formatDateTimeLabel(value.endDateTime)}`
}
...
export default function DateTimeRangeSelector({
  ...
  const [viewDate, setViewDate] = React.useState<Date>(value?.startDateTime ?? new Date())
  ...
  const commitValue = (
    nextDateRange: { startDate: Date | null; endDate: Date | null },
    ...
    setValue?.(nextValue)
    if (nextValue.startDateTime && nextValue.endDateTime) setOpen(false)
  }
  ...
  const parsed = parseTimeParts(activeField === 'start' ? timeRange.startTime : timeRange.endTime)
  ...
  const displayRange: DateTimeRangeValue = {
    startDateTime:
      dateRange.startDate && timeRange.startTime
        ? combineDateAndTime(dateRange.startDate, timeRange.startTime)
        : null,
    endDateTime:
      dateRange.endDate && timeRange.endTime ? combineDateAndTime(dateRange.endDate, timeRange.endTime) : null,
  }
        ...
        <div className="absolute left-0 top-full z-50 mt-2 w-[360px] rounded-xl border border-zinc-800 bg-zinc-950 p-3 shadow-2xl">
          ...
          {currentView === 'date' && (
            <div className="grid grid-cols-7 gap-1">
              ...
              {days.map((day) => {
                ...
                const isStart = dateRange.startDate ? isSameDay(day, dateRange.startDate) : false
                const isEnd = dateRange.endDate ? isSameDay(day, dateRange.endDate) : false
                const inRange =
                  dateRange.startDate && dateRange.endDate && isBetweenInclusive(day, dateRange.startDate, dateRange.endDate)
                ...
                return (
                  <button
                    ...
                    disabled={disabled}
                    onClick={() => {
                      let nextRange: typeof dateRange
                      if (!dateRange.startDate || (dateRange.startDate && dateRange.endDate)) {
                        nextRange = { startDate: day, endDate: null }
                      } else if (isBeforeDay(day, dateRange.startDate)) {
                        nextRange = { startDate: day, endDate: null }
                      } else {
                        nextRange = { startDate: dateRange.startDate, endDate: day }
                      }
                      setDateRange(nextRange)
                      commitValue(nextRange, timeRange)
                    }}
                    className={cn(
          ...
          <div className="mt-4 border-t border-zinc-800 pt-4">
            <div className="mb-3 grid grid-cols-2 gap-2">
              ...
              >
                Start: {timeRange.startTime ? formatTimeValue(timeRange.startTime) : '--:--'}
              </button>
              <button
                type="button"
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            const nextTimeRange =
                              activeField === 'start'
                                ? { ...timeRange, startTime: nextValue }
                                : { ...timeRange, endTime: nextValue }
